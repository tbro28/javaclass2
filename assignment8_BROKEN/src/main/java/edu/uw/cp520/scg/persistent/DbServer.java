...
/**
 * Provides a programmatic interface to store
 * and access objects in the database.
 *
 */
public class DbServer {
    ...
    public void addTimeCard(TimeCard timecard) {
        ...
        String nonbillableHours = "INSERT INTO non_billable_hours (" +
                "account_name, timecard_id, date, hours) VALUES (?, ?, ?, ?)";
        ...
        String billableHours = "INSERT INTO billable_hours (" +
                "client_id, timecard_id, date, skill, hours) VALUES (?, ?, ?, ?, ?)";
        ...
        try(
                ...
                PreparedStatement psTimecard = connection.prepareStatement(
                        timecardQuery, Statement.RETURN_GENERATED_KEYS);
            ...
            for(ConsultantTime consultantTime : timecard.getConsultingHours()) {
                if (!consultantTime.isBillable()) {
                    psNonebillable.setString(1,
                            ((NonBillableAccount)consultantTime.getAccount()).name());
                    psNonebillable.setInt(2, timeCardId);
                    psNonebillable.setDate(3,
                            Date.valueOf(consultantTime.getDate()));
